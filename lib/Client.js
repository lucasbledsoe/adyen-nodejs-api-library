...
module.exports = class Client{
  ...
  requestApi(serviceName, servicePath, action, request){
    return new Promise((resolve,reject) => {
      const hostName = this.determinehostName(serviceName, servicePath);
      const path = servicePath + serviceName + "/" + API_VERSION + '/' + action;
  ...
  _handleResponse(httpResult){
    ...
    return new Promise((resolve,reject) => {
      ...
      if (statusCode !== 200){
        ...
        this._handleHttpError(adyenResult, url).catch(reject);
      } else{
        try {
          let parsedRes = JSON.parse(rawResponse);
          Object.assign(adyenResult,parsedRes);
          ...
          this._handleHttpError(adyenResult, url).catch(err=>{reject(err);});
        }
      }
    });
  }

  _handleHttpError(adyenResult, url){
    return new Promise((resolve, reject) => {
      var err;
